...
def test_runner_merge_gap_when_extratime_reduced(tmp_path):
    ...
    rd.mkdir()
    spec = RunSpec(
        scenario="scenario_0", model="IDM", pcav=0.5, vehicle_count=10, seed=1,
        run_id="merge-gap-test", pipeline_version=PIPELINE_V4_1, schema_version="2",
        sumo_seed=101, cav_count=5, requested_pcav=None, with_internal=True,
        ssm_capture_ttc_threshold_s=5.0, ssm_extratime_s=1.0,
        detector_frequency=60, edge_data_frequency=60, simulation_end=360, warmup=60,
        step_length=0.1, loops=4, network_file="net/scenario_0/loop.net.xml",
    )
    ...
    for f in ("routes.rou.xml", "performance.xml", "performance_HV.xml", "performance_CAV.xml",
              "emissions.xml", "emissions_HV.xml", "emissions_CAV.xml",
              "vehroute.xml", "lanechange.xml", "stderr.log", "ssm.xml",
              "detector_lane0.xml", "detector_lane0_HV.xml", "detector_lane0_CAV.xml"):
        (rd / f).write_text("<root/>")
    status = {
        "run_id": spec.run_id, "pipeline_version": PIPELINE_V4_1, "status": "SUCCESS",
        "return_code": 0, "run_spec_sha256": spec.sha256(), "schema_version": "2",
        "config_sha256": "", "network_sha256": "", "experiment_id": "",
        "sumo_seed": 101, "route_file_sha256": "", "vehicle_type_map_sha256": "",
    }
    ...
    def tracking(fpath, type_map, warmup_period=600, ttc_threshold=3.0, drac_threshold=3.0,
                 fragment_merge_gap_s=0.0):
        captured_gap.append(fragment_merge_gap_s)
        return _original(fpath, type_map, warmup_period, ttc_threshold, drac_threshold,
