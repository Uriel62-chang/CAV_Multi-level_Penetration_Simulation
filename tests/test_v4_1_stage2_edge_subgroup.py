...
def _assert_additive(all_val, hv_val, cav_val, rel_tol=5e-5, abs_tol=1e-9):
    ...
    assert rel_err <= rel_tol, f"HV+CAV={summed}, all={all_val}, rel_err={rel_err}"


def test_edge_perf_veh_km_additivity():
    all_r = parse_edge_performance(str(_BASE / "performance.xml"), warmup_period=60)
    ...
    _assert_additive(
        all_r["non_internal_edge_vehicle_km"],
        hv_r["non_internal_edge_vehicle_km"],
        cav_r["non_internal_edge_vehicle_km"],
    )
    _assert_additive(
        all_r["total_time_loss_s"],
        hv_r["total_time_loss_s"],
        cav_r["total_time_loss_s"],
        rel_tol=1e-3,
        abs_tol=1e-3,
    )
...
def test_edge_emis_additivity():
    all_r = parse_edge_emissions(str(_BASE / "emissions.xml"), warmup_period=60)
...
def test_detector_subgroup():
    det_all = [str(_BASE / "detector_lane0.xml")]
