...
> 版本状态：v0.4.1 的 pilot（162 runs）未过资源门禁，D-012–D-015 技术证据已闭合、治理状态未闭合，
> 因此不构成可发布版本。已决定**跳号发布**：v0.4.2 为下一发布目标，v0.4.1
> 的全部工程成果（本节省列）并入 v0.4.2 发布说明；本地 `v0.4.1` tag 已删除
> （2026-07-31，指向 `b16771e`，可随时按该 commit 恢复）。
