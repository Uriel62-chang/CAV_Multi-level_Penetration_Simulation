...
- **版本发布状态**：GitHub 最新公开版本 v0.4.0.post3；v0.4.1 为本地内部里程碑（未 push、tag 已删）；**v0.4.2 为发布目标（跳号发布，release notes 已就绪，待打 tag/发布）**；`raw_v0.4.2/`（约 34 GB）与 run-level CSV 外部保留（untracked，.gitignore 保护）；`out_v0.4.2/` 聚合/交接/分析/图已跟踪（Git，含 result_handover.json、raw_status_inventory.jsonl、result_analysis.md）
