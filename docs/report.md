...
1. **通行能力基本图成立且峰位随渗透率移动**：HV 峰位 k≈20、IDM 全 CAV k≈50、
   CACC 全 CAV k≈40，方向与理论临界密度（HV 17.4 / CAV 39.2 veh/km/道）一致。
...
（U55：自由流→临界→拥堵→近阻塞的完整密度–流量关系）重设计主网格。
...
- **峰位随渗透率移动**（s1/s2）：HV-only k≈20 → IDM p=1.0 k≈50 → CACC p=1.0
  k≈40，方向与理论 kc（17.4 / 39.2）一致；CACC 峰流量更高且峰位更靠近理论 kc。
- **s0 转角基线**：HV-only ~940 veh/h/道平顶（k≥20）——转角吞吐上限（~1,400 内）
  而非 τ 限制；全 CAV 峰值 1,794 / 1,857 @ k40。

![FD s3 瓶颈](../graph/v0.4.2/chart_fundamental_diagram_s3.png)

- **s3 瓶颈语义**：基本图为瓶颈排队–吞吐关系（非主线基本图）；HV 吞吐 ~500
  veh/h/道量级，CACC 全 CAV ~3,200——**2→1 合流冲突主导**，而非 τ 限制的 2,400。
...
- **基本图形态与峰位移动**：右偏山峰状基本图在各场景成立；渗透率升高推动峰位向
  理论 kc 移动、峰流量上升——CACC 在小间距（τ 更小）下扩展了自由流–临界区间。
- **s0 转角基线**：转角限速把通行能力钉在 ~1,400–1,900 之下（实测 ~940 平顶），
  是 s0→s1 几何平滑对照轴的设计内自变量，非缺陷；s0 的排放/TTC/时损被转角反复
  加减速放大。
...
2. FD 峰位随 cav 渗透率精确移动（HV k=20 / IDM-CAV k=50 / CACC k=40），基本图
   方案达成设计意图。
3. s3 高密度合流瓶颈反转成立且由流量口径稳健支撑；效率佐证按密度档分层使用。
4. 项目当前无 P0/P1 级代码逻辑缺陷（管线审查四轮收敛：2 项 P2 已修复、遗留 P1
   为测量固有限制已文档化闭环）。
...
2. **闭环有限环长效应**：低密度自由流档可能出现自发拥堵（phantom jam），自由流
   分支实测流量系统性低估（s1 k=10 较理论低 28%、峰值仅为理论 71%）——低估是
   闭环普遍现象，非 s0 专属。
...
6. **s3 圈计数选择偏差（k≥40）**：窗内完整圈覆盖率 k=40 93% / k=50 68% /
   ...
   k≤35，"瓶颈反转"主结论依赖流量口径。
7. **THW 为条件样本**：无 leader 样本（实测 s0 v10 ~19%，恰为最大间距样本）被
   排除出 THW——mean_thw_s 系统低估。
