...
- **v0.4.1.post1 已启动**：仅补强 acceptance/input 冻结、SHA 追溯与 SSM 最小复现诊断；canonical frozen pair 已接入非 dry-run v0.4.1 batch 的 `--acceptance`、manifest 与 resume 防覆盖闭环。SSM repro 工具已冻结 s2/s3 CACC 全 CAV `as00/ss102` 对照 case，保留原始输入/输出、RSS 序列与派生报告；s3 无事件会明确标记 positive-control failed 而不换 treatment。不修改 `v0.4.1` tag 或原验收阈值，不启动新正式网格，不倒灌追认原 pilot。
- **已验证**：201 tests passed；Ruff/mypy/format/compileall 通过；pilot 162 与 legacy 10,080 dry-run 通过；micro-pilot Level 1 通过（10/10）；Level 2 bounded factorial pilot 完成（162 runs, failed original resource gate）；mitigation calibration 完成（48 runs with extratime=1.0+merge, failed simultaneous fidelity+RSS gates）。
