...
### 2026-08 U90 重设计（基本图方案，配置已落地待重跑）

- **实验参数重设计**（P0 插入缺陷终审 + 交通流理论评估驱动）：密度轴扩展至
  90 veh/km/道（≈68% kj_HV，s0/s1 10–180、s2/s3 20–360，18 档/场景），
  主网格 **7,524 runs**（基本图方案：自由流→临界→拥堵→近阻塞完整形态）。
- **`departSpeed="max"` → `"0"`**（P0 修复）：饱和环路高密度档插入损失
  （实测 s1 v120 仅 62/120、s2 v240 132/240）消除，100% 插入；warmup=600
  保持（9 档标定全部 ≤120 s 稳定，覆盖 10–90 veh/km/道、纯 HV/混合、瓶颈）。
- **设计文档**：根目录 `EXPERIMENT_DESIGN.md`（口径、密度轴、标定、报告边界）。
- A 方案 7,524 为上一版设计（已取代，见下条历史记录）。
