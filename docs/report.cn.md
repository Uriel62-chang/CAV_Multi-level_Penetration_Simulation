...
（与内部 Roadmap 同步，2026-08 修订：v0.6.0 并入原 v0.7.0 通信降级为阶段 D，
v0.7.0 降级为远期预留。）
...
- **v0.6.0**：TraCI 动态调控 + **单交叉口自适应信控框架（首发应用）**——固定配时
  （阶段 A）→ 感应/自适应（阶段 B）→ CAV 渗透率 × 信控交互（阶段 C）→
  **通信降级 × 车路协同鲁棒性（阶段 D，原 v0.7.0 并入，以 Benefit Retention
  Ratio / Robustness Phase Diagram 量化收益消失边界）**；效率指标口径迁移
  （闭环圈时 → 行程时间/排队延误/控制延误）。
- **v0.6.1**：**综合收尾与整合**——五张核心图（标定/收益相图/机理/通信鲁棒性/
  Pareto）+ 最终敏感性分析。
- **v0.7.0**：**远期预留**——CACC 通信降级专项（丢包、时延），仅当 v0.6.0
  阶段 D 结论显示该维度值得独立专项时启用；默认并入 v0.6.0。
