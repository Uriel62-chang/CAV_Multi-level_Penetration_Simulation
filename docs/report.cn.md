...
> **项目名称沿革**：本报告随 v0.4.2 以原项目名 *CAV Multi-level Penetration
> Simulation* 发布；项目在 v0.4.2 后更名为 **CAV Mixed-Traffic Lab**。为保持发布
> provenance，本报告标题、正文引用、实验结果与结论均保留发布时原貌。
...
### 2.1 设计口径（用户确认，2026-08）

1. **密度上限取各场景近阻塞密度（非完全阻塞）**：体现基本图形态（山峰 + 右偏
   长尾），不测完全静滞段。
2. **`departSpeed="0"`**：确保 warmup 前形成稳定车流；闭环路网照应现实高等级
   路段（高速公路、城市快速路）。
3. **warmup 时长联立②确定**：由 SUMO 实测标定（§2.3），不按公式拍脑袋。
...
- **仿真窗**：warmup=600 s（9 档实测标定 ≤120 s 稳定，见下表）、
  simulation_end=1800 s、观测窗 [600, 1800)。

**warmup 标定（2026-08，9 档实测，`departSpeed="0"` 下 FCD 1s 全程记录）**：
...
（全部 ≤120 s 达到稳态，采样粒度 120 s；warmup=600 s 余量 5 倍；“中间密度更慢”
假设被实测否定——s2 v160 最稳；CAV 混合稳定不慢于纯 HV。）
...
**设计论证（并入自 EXPERIMENT_DESIGN，原件移入 docs/internal/ 本地维护）**：

- **密度上限的内存约束（2026-08 实测）**：s2 全 CAV 峰值 v160=9.51 GiB、
  v200=22.69 / v220=22.67 GiB（均 SUCCESS，贴 24 GB+swap 边缘）、v230+ >22.7
  GiB（OOM）——SSM 设备对车辆对的跟踪开销随密度增长；故统一密度上限 55
  veh/km/道（v220 双车道 / v110 单车道），基本图右端从原 U90 的 68% kj 收缩至
  37.5% kj（仍覆盖临界 17.4 与拥堵区中段；“近阻塞”语义改为“拥堵区上探受限”，
  报告声明）。
- **3 workers 定案**：内存约束下固定并发最优——事件驱动模拟 3w 失败 5.6%/24h
  vs 6w 50%/32h（s2 高内存段 2–3 个并发即满）；实测 0 失败 / 21.86 h。
- **设计决策记录（2026-08）**：①口径 近阻塞密度（非完全阻塞）+ departSpeed=0 +
  warmup 实测标定——用户确认；②方案选 U90（68% kj）后经内存实测收缩至 U55
  （37.5% kj）；③s3 与主场景同轴，报告单独成图标注瓶颈语义；④warmup=600 定稿；
  ⑤main.json 落地 U55（7,524 runs，config_version=v0.4.2-main-7-U55-v220cap）。
...
- **s0/s1/s2 无 s3 式全档反转**（status 分类意义上无 reversal 档：s0 11 档密度全
  gain/mixed；s1/s2 5 gain + 4 mixed + 2 no-crossing）——但 **s1/s2 高密度 k≥45 档
  亦全渗透率无收益**（no-crossing：Δq_model 全程为负，性质与 s3 反转档相同，只是
  曲线负贯穿无正→负交叉）；flow Δq_model 跨 seed 一致性分布全场景均以 reversal
  占优（s0 58/41/11、s1 53/47/10、s2 50/51/9，reversal/mixed/gain，110 组/场景）；
  Δq_model 等权均值为负（s0 −26 / s1 −93 / s2 −99 veh/h/lane）——**峰值更高与均值
  更弱并存**，两结论由不同口径支撑（峰值 vs 等权均值）。
- **s3 瓶颈反转（核心信息）**：高密度 k∈{30,40,45,50} 检出 **reversal**
  （`p* ≤ 0.1` 且 `p_reversal_start ∈ (0.1, 0.2]`——**p≥0.2 任意渗透率混合在强制
  合流高密度下吞吐即反转**；p=0.1 边际档点估计微正（+3.5…+25.3 veh/h/lane）但
...
- **Pareto（四维：max flow / min delay / min conflict / min CO₂，无人工权重；
  ...
  （60 组）且更偏向低-中渗透率与 IDM 配置**——front 点 pCAV 均值 0.473（四场景
  最低）、p≥0.6 占比 33.3%（s0/s1/s2 为 76.8%/59.3%/60.4%）、IDM 配置占比 76.7%
  （最高）：瓶颈场景高渗透率 CACC 的 flow/delay 劣势使其在组内被支配；s0/s1/s2
  高渗透率档在 front 中占比更高——不同场景存在不同的 Pareto optimal region。
- **敏感性**：口径替代检验下 109/132 组合 p* 档位不变——拆分：flow total vs
  per-lane 44/44 恒不变（场景内车道数恒定，为尺度一致性检查，恒真属预期）；
  delay mean vs p95 34/44、TTC vs DRAC 31/44 为具判别力对比——阈值结论对口径
  选择稳健。
...
4. 项目当前无 P0/P1 级代码逻辑缺陷（管线审查 18 轮收敛封卷：代码层收敛、
   数据/文档层与发布/产物状态复核闭环；所有 P1/P2 均已修复或文档化闭环，
   无 open 项）。
...
（与内部 Roadmap 同步，2026-08 修订：v0.6.0 拆分为三个阶段版本——v0.6.0 阶段 A、
v0.6.1 阶段 B+C、v0.6.2 阶段 D；原 v0.6.1 收尾上移为 v0.7.0，原 v0.7.0 远期预留
上移为 v0.8.0。）

- **v0.5.0**：真实轨迹驱动的 **HV** 跟驰模型标定与仿真验证——NGSIM/HighD 高速
  直道数据标定 HV 跟驰参数；CAV（IDM/ACC/CACC）维持设定/文献参数，形成
  "HV 真实基线 vs CAV 模型设定"对照；SUMO 复现验证 + 泛化边界报告。
- **v0.5.1**：**HV 异质性与 CAV 空间组织机制**（机理研究）——按标定分布聚类
  Aggressive/Normal/Conservative 驾驶行为；引入 CAV 空间组织维度
  （random/clustered/dispersed，相同渗透率不同排列），以换道/速度波动/排队/
  合流压力等过程指标解释瓶颈反转的**形成机制**。
- **v0.6.0**：**TraCI 底座 + 效率指标口径迁移 + 固定配时基线（阶段 A）**——引入
  TraCI 作为解耦控制接口；单交叉口路网生成 + OD 流生成；效率指标口径迁移
  （闭环圈时 → 行程时间/排队延误/控制延误）；固定配时信号基线（口径迁移后的
  测量框架先经独立验证，再叠加任何信控算法）。
- **v0.6.1**：**信控科研（阶段 B+C）**——感应/自适应信控 → CAV 渗透率 × 信控
  交互，量化 CAV 信息相对无 CAV 信息的感应/自适应基线的增量收益。
- **v0.6.2**：**通信降级鲁棒性（阶段 D）**——丢包率/通信延迟/范围衰减 × 车路协同
  收益，以 Benefit Retention Ratio / Robustness Phase Diagram 量化收益消失边界。
- **v0.7.0**：**综合收尾与整合**——五张核心图（标定/收益相图/机理/通信鲁棒性/
  Pareto）+ 最终敏感性分析。
- **v0.8.0**：**远期预留**——CACC 通信降级深挖专项，仅当 v0.6.2 阶段 D 结论显示
  该维度值得独立专项时启用；默认并入 v0.6.2。
